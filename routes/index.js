...
const constructorMethod = app => {
  ...
  app.post('/edit-profile', loggedIn, async (req, res) => {
    ...
    if (data.fname == "" && data.lname == "" && data.email == "" && data.gender == "" && data.dob == "") {
      //res.status("400");
      res.render('edit-profile', { id: req.session.user.id, user: req.session.user, name: name, status2: "Data Not Changed" });
      return;
    }
    ...
    try {
      ...
      res.render('edit-profile', { id: req.session.user.id, user: req.session.user, name: name, status1: "User's data updated successfully" });
      return;
    } catch (e) {
      // res.status("400");
      console.log(e);
